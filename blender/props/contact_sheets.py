...
SHEETS: dict[str, dict] = {
    # the davit and crook arms reach 4.5 m along +Y; at the sheet azimuth that is ~2 m of screen width, so these
    # sheets get a wide pad instead of a side-on camera (which would make the row overlap itself)
    "lighting": {"ids": ["lamp_cobra_davit", "lamp_bishops_crook", "lamp_park_twin"], "pad": 4.4},
    "lighting_night": {"ids": ["lamp_cobra_davit", "lamp_bishops_crook", "lamp_park_twin", "subway_globe_green",
                               "subway_globe_red"], "pad": 3.4, "night": True},
