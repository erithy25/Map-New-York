...
def use_textures(pairs: Sequence[tuple[str, str]], *, maps: Sequence[str] = ("color", "normal", "roughness")) -> dict[str, str]:
    ...
    picks up the textured version. UVs are metres (``nycsim_bpy.extrude_polygon``/``MeshBuilder`` emit none, so the
    glTF UV set is generated by Blender's default per-face mapping); ``uv_scale_m`` is the asset's measured
    ``physical_size_m``. Returns {palette_name: status}."""
    status: dict[str, str] = {}
    if not _TEX_ON:
    ...
    for pal, tex in pairs:
        ...
        try:
            ...
            nb.pbr_material(pal, base_color=C._srgb(*rgb), roughness=rough, metallic=metal, textures=use,
                            uv_scale_m=float(meta.get("physical_size_m") or 2.0))
            status[pal] = f"{meta['asset_id']} ({meta.get('provider', 'ambientcg')}, CC0)"
