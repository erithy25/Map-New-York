...
def build(lod: int = 0):
    ...
    # plaza oaks on a 7.3 m grid, clipped to the plaza and clear of the pools and buildings
    trees = []
    ...
    extras = {
        ...
        "fidelity_statement": (
            ...
            "published 400+, and they are b_common.simple_tree geometry, not prop-library assets — "
            "blender_out/props/ held no tree asset when this model was built. Not modelled: the PATH platforms, the "
...
def main() -> None:
    ...
    # Church Street runs along the east side of the site: the canonical Oculus viewpoint
    a = math.radians(bc.heading_to_math_deg(PLAZA_AXIS_DEG))
    n = Vector((-math.sin(a), math.cos(a), 0.0))
    church = (ocx + n.x * 95.0, ocy + n.y * 95.0, GRND + 1.7)
    ba.run_landmark(
        ID, TITLE, build, bins=BINS, budget_lod0=250_000, budget_lod1=60_000,
        renders=[
            dict(view="oculus_from_church_street", cam=church, target=(ocx, ocy, 22.0), fov_deg=62.0, context=ctx,
                 sun_azimuth_deg=250.0, sun_elevation_deg=40.0),
            dict(view="memorial_plaza", cam=(pa.x + d.x * 105.0 + 40.0, pa.y + d.y * 105.0 + 20.0, GRND + 1.7),
                 target=(pa.x, pa.y, GRND + 1.0), fov_deg=64.0, context=ctx, sun_azimuth_deg=200.0,
                 sun_elevation_deg=48.0),
            dict(view="site_aerial", cam=(-360.0, -560.0, 300.0), target=(0.0, 0.0, 90.0), fov_deg=52.0, context=ctx,
                 sun_azimuth_deg=210.0, sun_elevation_deg=45.0),
        ],
