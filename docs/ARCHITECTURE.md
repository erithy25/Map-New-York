...
* **Transit structures**: elevated subway and Metro-North viaducts from OSM (`railway=subway|rail` with `bridge=yes` / `layer`) + planimetric transportation structures. ~~Trains run on GTFS schedules.~~ **They do not, and this line said they did.** Measured: `runtime/transit.nycb` holds exactly five sections — `bus_routes` (345), `bus_stops` (13,364), `route_stops` (21,963), `vertices` (137,060), `strtab` — and **no rail section of any kind**; the only "train" in the engine is `ANYCAmbienceEmitter::ScheduleNextTrain`, a *sound* whose interval is drawn from the real GTFS headway of a line that is not simulated. **Scope, stated rather than implied: rail in this build is surface features only** — 986 km of elevated structure you can drive under, 2,120 street entrances you can walk up to, station platforms, and the sound of a train passing overhead on its real headway. No rolling stock moves anywhere, above ground or below. `transit/rail_routes.parquet` (47 routes) and `rail_stops.parquet` (1,166 stops) are extracted and read by nothing but the ambience emitter's headway lookup (docs/DEVIATIONS.md D11). The buses **are** simulated: those five sections are what drives them.
