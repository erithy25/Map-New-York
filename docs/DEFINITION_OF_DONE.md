...
| 3 | Screenshots from the seven standard viewpoints compared with real photographs | 519 licensed photographs across 172 subjects in `docs/verification/reference/`; 57 comparison sheets with written assessments in `docs/verification/comparison/`; `test_the_mandated_viewpoints_and_drive_areas_all_have_a_usable_comparison`; `test_verification_renders_can_actually_serve_as_evidence`; `test_no_comparison_camera_is_placed_underground` | **done — 57 of 57 scenes** have a render, a sheet and a written assessment, and no `render_error.txt` remains. That covers all 9 mandated renders (seven viewpoints, two of them in two states), all 10 drive-through scenes across the five named areas, and 38 further landmark scenes beyond what the brief asks. **The assessments are candid and the answer they give is not photorealism**: the promenade calls its own skyline "a massing study rather than a city", the Woolworth frame cuts off the top 108 m of its subject, and two sheets compare views the reference photograph does not show (deviation I12). **Every sheet also shows an empty city** — 51 of the 57 assessments record no people and 37 no vehicles, because the verification scene places no agents, so the whole set understates the world in one direction (I13). See B12–B15 for the photorealism gap and I8–I13 for what the render pass itself found |
