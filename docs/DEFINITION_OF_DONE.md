...
| 4 | Traffic behaves as specified across the whole city | `core` traffic tests; `data/processed/traffic/density.parquet` 18,864 calibrated cells | **not met** — the C++ traffic and pedestrian suites still fail; the calibration data behind them is done and cross-checked |
