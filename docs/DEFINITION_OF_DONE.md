...
| 3 | Screenshots from the seven standard viewpoints compared with real photographs | 519 licensed photographs across 172 subjects in `docs/verification/reference/` with viewpoint and azimuth; comparison sheets in `docs/verification/comparison/`; `test_verification_renders_can_actually_serve_as_evidence` | **in progress, 50 of 57 scenes usable** — every drive scene, both Times Square scenes, all four bridges and 40 of the landmarks render with real structure; six Midtown/Financial District street-level scenes still come out near-black (NYSE 0.000, MoMA 0.005, MetLife 0.016, NYPL 0.021, St Patrick's 0.023, NY Times 0.035) and 9/11 Memorial Pools fails to render at all. The render gate fails on exactly those seven and is deliberately left failing |
