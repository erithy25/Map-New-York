...
| 3 | Screenshots from the seven standard viewpoints compared with real photographs | 519 licensed photographs across 172 subjects in `docs/verification/reference/`; comparison sheets and written assessments in `docs/verification/comparison/`; `test_the_mandated_viewpoints_and_drive_areas_all_have_a_usable_comparison`; `test_verification_renders_can_actually_serve_as_evidence` | **the mandated set is complete; the extras are not** — all **9 of 9** renders the brief names (seven viewpoints, two of them in two states) and all **10 of 10** drive-through scenes across the five named areas have a render, a sheet and a written assessment. Of the 48 further landmark scenes the lane added beyond the brief, six still render near-black and one not at all, so 50 of 57 total are usable and the render gate is left failing on those seven. **The assessments are candid about quality and it is not photorealism**: the promenade assessment calls its own skyline "a massing study rather than a city" — see deviations B12–B14 |
