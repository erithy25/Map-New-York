...
## ADR-022 A street or plaza on structure is a platform deck: extent from real polygons, elevation from the survey, burned only where it stands above the ground
Context: The 1 m 3DEP product is bare earth. A viaduct deck is classified out of it and anything built after the 2013-14 flight is absent, so under Tenth Avenue across the West Side Yard the heightmap held the yard's cut (−0.90 m at NYC_TM (−4245.81, 5899.01)) where the avenue's own spot elevations read 5.17-9.20 m; CSCL codes the avenue at grade, so `apply_terrain_z` draped segment 1267 into the cut (5.15, 0.60, −0.65, 9.39 m) and the whole Hudson Yards public square sat 6-10 m under its survey (DEVIATIONS J85). ADR-018 could not reach it: a yard floor at +2.8 m is above the −2 m floor, and the real street points 5-10 m above the DEM are exactly what `densify`'s 3 m outlier test rejects. The tempting fix — burn the landmark catalogue's 7.8232 m — is a mean of six footprint grounds across two epochs, not a survey of the deck, and a flat deck would step Tenth Avenue 2.5 m at W 31st.

Decision: a register of **platform decks** (`terrain/platforms.py`, `terrain/platform_decks.parquet`, DATA_CONTRACTS §3.1) enters the terrain stage through the same deck step as the pier and jetty decks. Each row names its extent as real polygons only (DoITT roadbeds, the road network's own intersection nodes, a DoITT bridge polygon; a sidewalk polygon is cut to its frontage on the named carriageway) and its elevation as the survey only: the inverse-distance-squared mean of the eight nearest survey points within 240 m — `survey_fill`'s points, weights and reach, without its growing-radius rule, which on a road surveyed every 25-55 m makes nearest-neighbour plateaus with 1.6 m risers (measured). Footprint LiDAR grounds inside an extent are excluded (their epoch is the excavation's or unknown: the Vessel's is 2.74 m, 15 Hudson Yards' 9.14 m). The deck is burned only where it stands above the ground (`z = max(z, deck)`): a platform spans a hole, and where the flight already measured ground at deck level that measurement is kept — `max` is continuous, so nothing steps at that boundary. Every replaced sample is counted per deck in `terrain.json`. The same function applies to a published tile (decode, burn, re-quantise), agreeing with a rebuild to one 2.5 mm quantum inside the extent, because the 2 m source mosaic was deleted for disk (`src2m_removed.json`) and a rebuild needs a 525 MB re-download onto 1.6 GB of free disk.

Consequences: Tenth Avenue grades 5.40 → 9.39 m through the yard and its 15 vertices, lanes and node stand on the survey; the square reads its 'New' spot elevations (10.0-12.3 m) exactly; every input is global, so the 5,724 tile seams still agree to 2.8 × 10⁻¹⁴ m. What the rule does *not* decide: the platform's level under the buildings is interpolated from the perimeter and the square's spots (podium median 8.79 m) and is now **1.06 m above** the catalogue's 7.8232 m, so the landmark ground test still refuses the plaza — in the other direction — and the catalogue datum is the next decision, not a terrain nudge. Eleventh Avenue's bridge over the yard joined the register on the same evidence; Manhattan West and W 33rd's regrading are measured and named as candidates without an extent.
