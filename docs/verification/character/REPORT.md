...
`wardrobe.py` cuts each garment out of the character's own skin: the body vertices in the garment's region
(selected by dominant deform bone and a height band) are copied, pushed out along their normals by the
fabric stand-off, solidified to the fabric thickness, and given a hem and cuff by tapering the stand-off to
30 % over the outer 6 % of the garment's height span. Layer stand-offs are 6 mm (base) / 13 mm (mid) /
24 mm (outer) so an outer garment always clears the outer *surface* of the one below it. Puffers add a
cosine quilt channel; hoodies add a hood ellipsoid weighted 55/45 to `neck_01`/`spine_05`; shoes get 18
Laplacian smoothing passes before the offset (which merges the toes into a toe box) plus a sole slab.

Two properties follow from cutting the pattern out of the body:

* **it fits every body** the NPC generator produces, including the two children, without a per-body fit step;
* **the weights are exact** — each garment vertex takes the weights of its nearest body vertex through a
  KD-tree, so a sleeve bends exactly like the arm inside it and the 4-influence/sum-1 invariant is inherited.

The player wears `tee_white`, `jeans_indigo`, `hoodie_grey`, `jacket_bomber`, `sneakers_black` and a
procedural steel watch (38 mm case, strap radius measured from his own forearm vertices) on the left wrist.

**Gap:** these garments have no seams, plackets, zips, buttons, collars or lapels, and no cloth simulation.
They read correctly in silhouette and deform correctly, and a puffer reads as a puffer, but at close range
they are offset shells of a body, not tailored panels. Fixing that properly needs either a pattern-drafting
step or hand-modelled garments; neither was in reach here. It is the largest visual gap in this lane after
hair.
...
* **40 wardrobe items** (`wardrobe.WARDROBE`): 12 tops, 4 mid-layer hoodies, 14 outerwear (3 puffers plus a
  long puffer, 3 jackets, 2 coats, 2 suit jackets, ANSI hi-vis, Con-Ed orange, insulated delivery vest),
  8 bottoms (jeans x2, chinos, suit trousers, scrubs, joggers, pencil skirt, kid's jeans), 4 shoes, and the
  hijab. Scrubs, hijab, delivery vest, hi-vis, tourist tee and three children's items are all present;
...
The renders were opened and acted on, not just produced. Six defects were found this way and fixed:
...
4. three coincident garment shells read as one shapeless mass, and a ring of bare skin showed at the waist
   where no garment's bone region covered the pelvis; layer stand-offs and the tops' bone regions were fixed;
...
4. **Garments are offset shells** with hems and quilting but no seams, zips, collars or cloth simulation (7).
