...
**In the scene**, within 776.2 m of the camera and not all of it in frame — 4 building tiles (106,490 tris), 14 landmark models of which **5 can fall inside the 54.4° frame**, 28,368 pavement polygons (12,518 white, 5,222 sidewalk, 4,534 roadbed, 3,852 curb, 787 plaza, 755 crosswalk, 376 median, 290 yellow, 34 parking lot), 1,025 props of the 6,174 in range, 3,279 kit pieces, 89 vehicles and 242 people; 4,500,119 triangles. Ground mesh 88,976 triangles, 0 holes. 20 city surfaces are dressed from the shared photographic catalogue.
...
* **Contrast and colour, which here measure the reference, not the city.** Standard deviation **0.1442** against **0.2729** (ratio **0.528**; `render.json`'s frame block holds **0.1446** for the same render), 95th percentile **0.6735** against **0.8893**, chroma **0.0458** against **0.112** (**0.409**). The photograph's bright tail is the flag's white stripes and sunlit stone; its chroma is the flag and the sky. The facades' one hue per class (J66) is visible in the flat tan slabs, but this pairing cannot measure it.
...
* **What the budgets removed.** **1,025** of 6,174 props placed (triangle budget 1095576); **1,232** pedestrians and **69** vehicles dropped for theirs; the kit capped at 949947 triangles. **173** of 188 trees are species-substituted (J70); 11 artworks, 9 memorials and 4 drinking fountains have no asset (J23); **57** scaffold pieces are from present-day permits, against a May 2023 photograph.
...
| sd 0.528×, p95 0.6735 against 0.8893, chroma 0.409× | the photograph's bright and coloured pixels are a flag, sunlit stone and sky the render's frame does not contain | reference — **DEVIATIONS I12** |
...
`scene.agents.seconds` 551.98 -> 558.17,
`scene.seconds` 648.72 -> 653.43,
`seconds.render` 174.1 -> 168.9,
`seconds.scene` 652.6 -> 657.6,
`seconds.total` 826.7 -> 826.5.



## Measured for this assessment

The wall-clock figures in the table below are the **previous** render's, quoted so that "every measured value is identical" is a comparison a reader can check rather than a claim. They were read from this sheet's own `render.json` at commit `3019323`, its last state before the pass restarted.

| figure | where it comes from |
|---|---|
| 551.98 | `scene.agents.seconds` in the previous record; the re-render took 558.17 s |
| 648.72 | `scene.seconds` in the previous record; the re-render took 653.43 s |
| 174.1 | `seconds.render` in the previous record; the re-render took 168.9 s |
| 652.6 | `seconds.scene` in the previous record; the re-render took 657.6 s |
| 826.7 | `seconds.total` in the previous record; the re-render took 826.5 s |
