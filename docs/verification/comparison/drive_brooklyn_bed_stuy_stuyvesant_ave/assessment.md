...
**Sun** — azimuth 249.3°, elevation 30.5° at 2013-09-07T16:30:00-04:00 (the photograph's own categories carries the date, and 16:30 **chosen**, not measured: of the hours that put the Sun above 20 deg it is the one whose bearing (249 deg) comes closest to the view azimuth (351 deg), 102 deg off, so the Sun is behind the camera and lights what it looks at); 774.8 W/m² direct normal, sky at strength 0.0356, Filmic, +3.68 stops.

**In the scene**, within 766.3 m of the camera and not all of it in frame — 6 building tiles (670,572 tris), 0 landmark models, 19,150 pavement polygons (7,648 white, 4,326 sidewalk, 3,340 roadbed, 2,895 curb, 451 crosswalk, 212 parking lot, 192 yellow, 63 median, 23 plaza), 3,097 props of the 3,204 in range, 4,123 kit pieces, 73 vehicles and 376 people; 4,500,138 triangles. Ground mesh 89,888 triangles, 0 holes. 18 city surfaces are dressed from the shared photographic catalogue.
...
**The two halves are the same street and not the same view.** The camera stands on the photograph's own EXIF GPS, **46.3 m** from the recorded viewpoint at the Stuyvesant Avenue and Decatur Street crossing. That GPS is on Decatur Street; the azimuth is **351.4°**, Stuyvesant Avenue's bearing from `meta.json`, because the item names no subject. So the render looks north across Decatur Street at its north-side row, clear for **23.1 m**: four stoops, four arched doorways, no roofline. The photograph is an oblique down the same row under a blown overcast sky, three SUVs at the kerb. Compare on street width, storey height and material, not composition; the previous reference was a photograph of a different corner (J60).

**The date is the photograph's own now.** This sheet was lit on the assumed summer solstice at 18:30; J114 made `photo_instant` read the photograph's own words, and **its Commons categories carry 7 September 2013** — so the render is lit on a September afternoon at 16:30, the Sun **10 degrees higher** than the assumed solstice evening gave it and 35 degrees further south. 7 September 2013 was a **Saturday**, and the crowd is now drawn for a Saturday where the old record drew a weekday. The hour is still chosen rather than measured and is now further off the view azimuth than before — **102°** against 67° — because a September afternoon cannot be got behind a camera facing 351° at any hour above 20° elevation (J80).

**The row is in direct sun, and the Sun the record prints is not the lamp that lights it.** The facade sits near the top of the tonal range, every window box and door surround throws a hard-edged shadow down and to the left, and the ginkgo at **15.6 m** lays crisp leaf shadows on the wall left of its trunk. That is light from high on the right, behind a north-facing camera — what the record's gloss promises. The recorded Sun at **249.3°** stands west-south-west, ahead of the camera, and would light nothing on a wall facing south. The lamp in `setup_world_and_sun` (`render_sheets.py`) is rotated tilt-then-yaw, which sends its beam *toward* the recorded azimuth, so light arrives from the opposite bearing; the Nishita sky takes the azimuth as given, so sky and lamp disagree by half a turn. The hour chooser's comment (J80) inverts the hemisphere the same way; the two cancel, and the row is lit as intended by a mechanism that contradicts its record. Neither inversion is in the register.

**The metering (J83) developed the road, not the row.** Linear median **0.014027**, **+3.68 stops** where the physical rule gave **+0.49**; the record calls the frame under-lit. The median pixel is the carriageway, two fifths of the frame in shade thrown from behind the camera; the sunlit wall sits near the linear p95 of **0.047531**, on the shoulder. Under-lit is true of the road, not the facade.

## What matches

* **Typology and storey rhythm.** Raised basement, a stoop to a parlour door under a round arch, windows in rows above, at the same scale in both halves. **267** door entries, **2,640** windows and **330** window accessories out of **4,123** kit pieces.
...
* **Three simulated figures, none in the photograph**: `agent_ped_936.0` at **16.1 m** with an orange box, a grey figure at the left edge, a third cut by the right edge.
...
* **Hard light from the wrong bearing.** Shadows fall down-left from high on the right; the recorded Sun at **249.3°** is ahead-left of the camera, and the overcast photograph has no cast shadows. The real date did not change this: the inversion is in the lamp's rotation, not in the instant.
* **The lighting, in the record's terms**: **+3.68 stops**, under-lit; the photograph sits at **-1.202** stops from the metering convention and the render at **0.244**, **1.45** stops apart. Mean **0.490** against **0.3992** is that gap first. p05 **0.163** against **0.0583** is shaded road lifted towards middle grey against black under the photograph's cars; p95 **0.7280** against **0.9972** is sunlit wall on the shoulder against blown sky.

## Cause of each gap

| gap | cause | class |
|---|---|---|
| one pale stone for four house colours; chroma 0.366× | one material family per facade class; `shellmat` varies tone, never hue | data, **DEVIATIONS J66** |
...
| under-lit at +3.68 stops, exposure 1.45 stops apart, p05 0.163 against 0.0583 | the median pixel is the shaded carriageway, metered to middle grey; the sunlit facade is pushed onto the shoulder | stated choice, **DEVIATIONS J83** |
...
`scene.agents.seconds` 704.74 -> 706.62,
`scene.seconds` 862.76 -> 855.87,
`seconds.render` 139.3 -> 137.7,
`seconds.scene` 870.0 -> 862.1,
`seconds.total` 1009.3 -> 999.8.



## Measured for this assessment

The wall-clock figures in the table below are the **previous** render's, quoted so that "every measured value is identical" is a comparison a reader can check rather than a claim. They were read from this sheet's own `render.json` at commit `e157abf`, its last state before the pass restarted.

| figure | where it comes from |
|---|---|
| 704.74 | `scene.agents.seconds` in the previous record; the re-render took 706.62 s |
| 862.76 | `scene.seconds` in the previous record; the re-render took 855.87 s |
| 139.3 | `seconds.render` in the previous record; the re-render took 137.7 s |
| 870.0 | `seconds.scene` in the previous record; the re-render took 862.1 s |
| 1009.3 | `seconds.total` in the previous record; the re-render took 999.8 s |
