...
**Camera** — 40.756779, -73.982545 (NYC_TM -2748, 6306) at z 20.1 m NAVD88 | azimuth 29.0°, pitch 0.0° | 35 mm on 36 mm (54.4° horizontal) | 1280x854. The camera stands on **this photograph's own EXIF GPS**, 35.3 m from the item's recorded viewpoint, and was **not moved**; the view azimuth is clear for 150 m. The ground under it reads 18.473 m NAVD88, the **10th percentile of 113 samples within 12 m**, range 18.25 to 18.87 m. The item names no subject, so the axis is level and the lens the default 35 mm, and the record directs the comparison: *"compare them on street width, storey height and material, not on composition."* The nearest built thing in the frame is **`prop_tree_pin_oak_small_bare_13` 13.5 m** away and **the nearest simulated agent is `agent_ped_2226.10` 8.2 m** from the lens at 9.1° off axis, to the left.

**Sun** — azimuth 115.5°, elevation 27.6° at 2018-03-22T09:30:00−04:00. **The date is the photograph's own and the hour is chosen.** Its Commons categories carry 22 March 2018, which `photo_instant` now reads (J114); of the hours putting the Sun above 20° on that date, 09:30 is the one whose bearing comes closest to the view azimuth — **115° against 29°, 86° off**, a wider miss than the assumed solstice's 56° because a March morning cannot be got behind a north-north-east view (J80). 746.9 W/m² direct normal, Filmic, **+5.81 stops**, clamped: *"the frame wanted +5.81 stops and was held at +5.81: a scene this far from a photographable level is not developed into a picture of one."* The linear median is **0.003217**. The physical rule would have given **0.64 stops**.

**And the real date changed the season in the picture.** 22 March is inside the leaf-off window — `props.leaf_off` is **true** where the assumed solstice made it false — so the street trees are the bare variants, and the tree that fills the render's right third is a naked pin oak rather than a leafy one. The nearest built thing in the frame is now that bare crown at **13.5 m**, where the previous pass named the leafed version of the same species at 8.0 m: a bare canopy takes less of the probe's cone. This is J114 reaching through to J97's canopy switch, and it is the one change in this pass that is visible at a glance in the picture.

**In the scene** — 4,500,100 triangles: 7 building tiles (315,514 tris), 9 landmark models of which 3 fall inside the 54.4° frame, 29,405 pavement polygons, 955 props, 4,480 kit pieces, 25 park-ground meshes, 53 vehicles and 242 people.
...
**The render is four people and a bare tree.** Three figures in green jackets and white trousers, one of them carrying a bag, and a fourth in a white top occupy the centre-left of the frame from thigh to head, the nearest **8.2 m from the lens**, with a leafless pin oak filling the right third and a dark glass tower behind it. Behind them a file of small pedestrians recedes along the sidewalk. The lower two-fifths is flat pale paving. No storefront, no signage, no bicycles, no hydrant.

**This is the third sheet in the pass where an agent standing beside the view axis takes the frame.** The Flatiron's near third is a delivery vehicle, the Oculus has two figures in its right foreground, and here the group is the subject. The clearance walk tested the view azimuth and found it clear for **150 m**, which is true of built fabric; an agent 8.2 m away at 9° off axis is not built fabric and the walk does not weigh it. The date change moved the nearest agent from 4.0 m to 8.2 m and so from waist-up to thigh-up in the frame, which is a smaller version of the same fault rather than a repair of it.
...
* **32 storefronts in the whole scene**, against **4,410 windows**. The photograph's entire ground floor is glass shopfront with signage, awnings and window displays. Kit was capped by a **741,974-triangle** budget with **30,816 pieces in range** — the frame wanted more than forty times what it drew, the worst kit shortfall measured in this pass, and shopfronts are what that removes.
...
* **The frame was clamped, not met**: it wanted **+5.81 stops** and was held at **+5.81**, so it is published below even its own metered target.
* **The render has more contrast and more colour than the day had.** Standard deviation **0.2785** against **0.2283** (**1.220×**), chroma **0.0594** against **0.0543** (**1.094×**), 95th percentile **0.9602** against **0.8939**. The reference is a flat overcast on wet asphalt; the render puts a 32° June sun into a clear sky and gets hard tree shadows the photograph has none of.
...
* **1,267 tree rows did not fit the props budget**, capped at **982,596** triangles.
* **The crowd is thin behind the near group.** The density table wanted **1,240 vehicles and 1,672.9 people**; **1,407 and 2,129** were simulated and **3,241** dropped — **892 pedestrians and 335 vehicles at the agent triangle budget**, 749 and 977 outside the radius, 153 pedestrians in the carriageway without crossing, 76 not on a walkable surface, 17 pedestrians and 2 vehicles above the observer, 18 vehicles not on a carriageway, and **22 riderless bodies**. The ask itself is larger than the previous pass's, because a 09:30 March hour draws a busier Midtown than 08:30 in June.
...
| gap | cause | class |
|---|---|---|
| four agents from 8.2 m fill the frame | the clearance walk tests the view azimuth for *built* obstruction and does not weigh an agent beside it; third sheet in the pass with this failure, after the Flatiron and the Oculus | **verification — open, and now a pattern** |
| 32 storefronts against 4,410 windows | kit capped at 741,974 triangles with 30,816 pieces in range, a shortfall of more than forty to one, and shopfronts are the first thing that removes | **performance — the worst kit budget in the pass** |
...
| the chosen instant is 86° off the view azimuth | no hour above 20° elevation on 22 March puts the Sun behind a 29° heading; the rule reports the miss rather than hiding it (J80) | reference — the date is now the photograph's own (J114), the hour is not |
| published at the +5.81 clamp having wanted +5.81 | the clamp's rule: a scene this far from a photographable level is not developed into a picture of one (J83) | verification — declared |
| sd 1.220×, chroma 1.094×, hard tree shadows | a flat overcast reference against a clear Nishita sky and a chosen June sun; nothing reads a historical sky | reference — no source exists |
| dry road against wet asphalt | no historical pavement condition is read | reference — no source exists |
| 1,267 tree rows dropped | the props triangle budget at 982,596 triangles | performance |
| 242 people against a table asking 1,672.9 | the agent triangle budget plus the placement rules, each with its count | performance + verification |
...
`scene.agents.seconds` 491.8 -> 490.85,
`scene.seconds` 635.33 -> 627.78,
`seconds.render` 161.8 -> 168.3,
`seconds.scene` 639.8 -> 632.0,
`seconds.total` 801.6 -> 800.3.



## Measured for this assessment

The wall-clock figures in the table below are the **previous** render's, quoted so that "every measured value is identical" is a comparison a reader can check rather than a claim. They were read from this sheet's own `render.json` at commit `31adbf4`, its last state before the pass restarted.

| figure | where it comes from |
|---|---|
| 491.8 | `scene.agents.seconds` in the previous record; the re-render took 490.85 s |
| 635.33 | `scene.seconds` in the previous record; the re-render took 627.78 s |
| 161.8 | `seconds.render` in the previous record; the re-render took 168.3 s |
| 639.8 | `seconds.scene` in the previous record; the re-render took 632.0 s |
| 801.6 | `seconds.total` in the previous record; the re-render took 800.3 s |
