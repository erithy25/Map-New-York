...
**Camera** — 40.754156, -73.980559 (NYC_TM -2581, 6014) at z 22.2 m NAVD88 | azimuth 29.0°, pitch 0.0° | 35 mm on 36 mm (54.4° horizontal) | 1280x854. The camera stands on **this photograph's own EXIF GPS**, **186.8 m** from the item's recorded viewpoint — inside the 250 m at which it can still be the same view, but only just — and was **not moved**; the view azimuth is clear for 150 m. The ground under it reads 20.603 m NAVD88, the **10th percentile of 113 samples within 12 m**, range 20.51 to 21.12 m. The item names no subject, so the axis is level and the lens the default 35 mm, and the record directs the comparison to street width, storey height and material rather than composition. The nearest built thing in the frame is `prop_sign_mta_bus_stop_5` 17.4 m away, and **the probe found no simulated agent on its rays across the frame** within 60 m — a change from the previous pass, which put `agent_ped_242.9` 4.6 m from the lens. Read that for exactly what it is: the probe is seven bearings by five elevations across the frame's own half-angles, not a sweep of a 60 m disc, and this scene places **261** pedestrians over a 200 m radius and **54** vehicles over 320 m — about **25** of them inside 60 m if they are spread evenly. The record's own sentence claimed the disc until J120 and no longer does; this one had repeated the claim. Nothing was culled over the observer here; the crowd is simply the one 18 May at 08:30 draws rather than the one the assumed solstice drew.

**Sun** — azimuth 90.0°, elevation 31.0° at 2018-05-18T08:30:00−04:00. **The date is the photograph's own and the hour is chosen.** `File:43rd St 5th Av td (2018-05-18) 21.jpg` carries 18 May 2018 in its own title, which `photo_instant` now reads (J114); the hour is the one whose bearing comes closest to the view azimuth — **90° against 29°, 61° off** (J80). 778.3 W/m² direct normal, Filmic, **+5.25 stops**, marked **under-lit**. The linear median is **0.004724**. The physical rule would have given **0.47 stops**.

**In the scene** — 4,500,036 triangles: 6 building tiles (339,572 tris), 13 landmark models of which **6 fall inside the 54.4° frame**, **36,632 pavement polygons**, 1,272 props, 3,593 kit pieces, 21 park-ground meshes, 54 vehicles and 261 people.
...
**The number that explains this sheet is 100,664.** That is how many kit pieces were in range; **3,593** were drawn, under a **618,902-triangle** budget. A shortfall of more than a hundred and sixty to one, by far the worst in the pass — and it is the single cause of almost everything the render lacks.

**The photograph's corner is under a green sidewalk shed** with `URBAN OUTFITTERS` along its fascia, scaffolding standards down the kerb, shop windows behind them, street signs on the mast, a Citi Bike dock at the right and red utility spray-paint on the asphalt. **The render has none of it.** Not one scaffold piece appears in this scene's kit at all, against **67 storefronts and 3,497 windows** — and other sheets in this pass carry 69 to 116 scaffold pieces, so the class is placed elsewhere and did not survive the budget here.
...
* **Contrast agrees almost exactly** — standard deviation **0.2171** against **0.2360**, a ratio of **0.920**.
* **The fleet is a Fifth Avenue weekday fleet**: **18 yellow taxis, 11 boro taxis, 10 sedans, 6 black cars, 5 box trucks, 2 SUVs, an MTA bus and a van**, with the crowd clock reporting **a weekday** for 2018-05-18, which was a Friday. The photograph has a bus, a box truck and a taxi in it.
* **This is one of the two sheets J114 named as datable from the string the record already printed**, and it now is. The date came out of the photograph's own filename rather than a stored field, and the render moved off the assumed solstice onto 18 May — a month earlier, the Sun a degree and a half lower and five degrees further round. On this sheet the change is small because 18 May and 21 June are both leaf-on weekdays with a high morning Sun; what it buys is that the frame no longer rests on an assumption the file itself contradicted.
...
* **310 Citi Bike dock units** are in range (Stage 40).
...
* **67 storefronts against 3,497 windows**, and **3 cornices, 3 pilasters, 3 string courses and 1 bulkhead** in the entire scene. At 100,664 pieces in range under a 618,902-triangle budget, the frame drew one piece in twenty-eight.
...
* **The Citi Bike dock is not in frame** although 310 units are in range.
* **The chosen instant is 56° off its own target**, and the frame still needed **+5.25 stops** and is marked under-lit.
* **The render is darker in the midtone and more colourful overall** — median **0.4994** against **0.5420** (**0.921×**), mean **0.5225** against **0.4557** (**1.147×**), chroma **0.0551** against **0.0432** (**1.275×**), 95th percentile **0.9451** against **0.7463**. The photograph is a flat overcast with no highlight near white; the render has a clear June sky and hard shadows.
...
* **1,852 tree rows did not fit the props budget**, capped at **916,361** triangles, and only **37** of the 431 trees placed are drawn from modelled branches within 120 m.
* **The crowd is a fraction of the ask.** The density table wanted **1,210 vehicles and 1,364 people**; **1,320 and 1,781** were simulated and **2,786** dropped — 815 vehicles and 738 pedestrians outside the radius, **623 pedestrians and 418 vehicles at the agent triangle budget**, 87 in the carriageway without crossing, 62 not on a walkable surface, 2 inside buildings, 8 above the observer, 1 not on a carriageway and **32 riderless bodies**.
...
| gap | cause | class |
|---|---|---|
| no sidewalk shed, no shopfronts, almost no cornice | 100,664 kit pieces in range against a 618,902-triangle budget — one piece in twenty-eight drawn, the worst shortfall in the pass | **performance — the binding constraint on this sheet** |
...
| the Citi Bike dock is not in frame | 310 dock units are in range and none falls in this 54.4° cone from this position | verification |
| the chosen instant is 61° off the view azimuth | no hour above 20° elevation on 18 May puts the Sun behind a 29° heading; the rule reports the miss (J80) | reference — no source exists |
| the frame needed +5.25 stops | a Midtown avenue in canyon shade at 08:30; published under-lit and marked so (J83) | reference + verification — declared |
| p50 0.921×, chroma 1.275×, a blown highlight the render has and the photograph has not | a flat overcast reference against a clear Nishita sky and a chosen June sun | reference — no source for a historical sky |
...
| 1,852 tree rows dropped, 37 modelled trees against 394 cards | the props triangle budget at 916,361 triangles | performance |
...
`scene.agents.seconds` 510.14 -> 479.54,
`scene.seconds` 620.07 -> 561.4,
`seconds.render` 173.3 -> 214.5,
`seconds.scene` 624.8 -> 563.9,
`seconds.total` 798.1 -> 778.3.
...
`cached -2581_6014_8_0_ab356ee7049bd58d_1788868452.json`).
...
|---|---|
| 510.14 | `scene.agents.seconds` in the previous record; the re-render took 479.54 s |
| 620.07 | `scene.seconds` in the previous record; the re-render took 561.4 s |
| 173.3 | `seconds.render` in the previous record; the re-render took 214.5 s |
| 624.8 | `seconds.scene` in the previous record; the re-render took 563.9 s |
| 798.1 | `seconds.total` in the previous record; the re-render took 778.3 s |
