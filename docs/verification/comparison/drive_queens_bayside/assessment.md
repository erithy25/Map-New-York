...
**The item asked for a residential street and its reference is a sunken six-lane Interstate.** The fetch recorded its own queries — *"Bayside Queens houses residential street"* and *"Bayside, Queens" house* — and kept Interstate 295 from the 42nd Avenue footbridge, a Flickr picture titled *"Needs Work"* and *Lawrence Cemetery*. None is a residential block. This is the sheet docs/DEVIATIONS.md **J71** was written from, and the row is open: the chooser ranks on lighting, timestamp and distance and has no test of what a photograph is a picture *of*. The Interstate won because it alone carries a real EXIF instant and a GPS fix; that fix is **566.6 m** from the recorded viewpoint, past the 250 m band, so the camera was correctly not stood on it (the J60 shape: the photograph is where its title says, the pairing is wrong).

**The two halves share nothing but the Sun.** The photograph looks south from a footbridge a storey above the Clearview Expressway: three lanes each way, a concrete median carrying cobra-head lamps on a Y-pole, yellow edge lines and dashed white lane lines, tan retaining walls with railings, trees over the walls, a red saloon in the near lane, a dozen-odd vehicles receding to an EXIT 5 sign, and a pale clear sky over the top third. The render stands at eye level on a broad grey apron beside a wide street running north: two- and three-storey red- and tan-brick taxpayers with green shopfront frames, a brick house with a stoop at the far left, a City flag on a pole mid-frame, a cobra-head lamp, three dark saloons and an SUV, two pedestrians, honey locust crowns at both upper corners and a deep blue sky. Street width, storey height, framing and material cannot be compared across that gap; the render's failure to resemble the photograph is not a fault of the model.

**Where the camera stands is its own small gap.** The item's note says *215th Street near 42nd Avenue, Bayside, looking north*; the record finds that point **0.1 m** from `t_15_7_struct_station_house`, so the camera was walked **32.2 m** onto the nearest sidewalk polygon. The view azimuth is clear for **60 m** and the nearest built thing in frame is `prop_tree_honeylocust_large_22` at **19.4 m** (**-27.2°** yaw, **+21.1°** pitch). The move is correct, and it leaves the frame on a shopfront strip beside a station house rather than the residential block the item is named for. No subject is named, so `sightline.subject_visible` is null; no J74, J78 or J79 question arises. What can be judged is whether the render is a plausible Bayside commercial street on a June Tuesday afternoon, and on that narrow question it holds.

## What matches

* **The instant is the photograph's own and the light agrees with it.** The Sun is placed from EXIF `DateTimeOriginal`, **2024-06-18 14:45:53**, azimuth **241.3°**, elevation **61.3°**, **921.7 W/m²** direct normal. Both halves show a high afternoon sun with short hard shadows — the photograph's lamp-pole shadow on the southbound lanes, the render's cars throwing theirs right and away from a north-facing camera with the Sun behind its left shoulder — under a clear sky. The crowd was drawn for **a weekday**, which the Tuesday is.
...
* **Nothing was cut for budget.** **4** of **4** tiles, 0 missing, 0 LOD-substituted, 0 holes, no kit cap; the **80** prop rows not placed are the curb ramps built into the pavement.
* **The traffic is the simulation's**: **73** vehicles and **265** people from seed **20260907**; the three cars and two walkers in frame are that density, not the photograph's.
...
* **The lower two-fifths of the render is a blank grey apron.** The record holds **7,333** white and **622** yellow marking polygons in range (J52 done); in frame only a short yellow dash at the far kerb is visible.
* **The development is metered, not under-lit.** The frame needed **+1.21** stops to bring its median to middle grey (unclamped **+1.21**; the physical rule alone would give **+0.00**), well below the +4 at which the record calls a scene under-lit (J83). The photographer exposed brighter of the same convention: `exposure_offset_stops` **0.528** against the render's **0.242**, recorded as **-0.286**. The mean ratio **0.749** (**0.4221** against **0.5632**) and median ratio **0.913** (**0.4988** against **0.5465**) are first that choice; the p05 pair, **0.1145** against **0.113**, is not a gap.
...
* **Chroma is about six-tenths of the photograph's**: **0.0646** against **0.1111**, ratio **0.581**. A red Civic, full-width foliage and blue sky on one side; grey pavement, black cars and two brick tones on the other. `roof_membrane` and `wood_clapboard` sit at the albedo cap, neither in frame; every building of a class is one hue at different brightnesses (J66).
...
* **Every agent is at LOD2**; the pedestrian beside the SUV is a low-detail body.

## Cause of each gap

| gap | cause | class |
|---|---|---|
| an Interstate for a residential street, footbridge eye against street eye | the fetch kept nothing answering its houses query and the chooser cannot reject a subject it has no test for; the overpass 566.6 m away is refused as a camera position | **reference — open, DEVIATIONS J71**, J60 |
...
| a blank grey apron across the lower two-fifths | the walk put the camera on a wide pavement polygon and the level axis puts the road high in the frame | verification |
...
| 1,174 trees species-substituted, 15 impostor cards dropped | no modelled species matched; the nearest by size and taxon is drawn at the census height | data |
