...
* **No structures at all.** **0 tiles imported, 4 without a file, 0 triangles**, on a block that sits over the busiest subway interchange in the system. Nineteen of the 171 records in the pass report the same, each with four tiles that have no structures file — counted over every record carrying `scene.structures`, which corrects an earlier reading of this line that called it the only such sheet.
