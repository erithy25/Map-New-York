...
**And the Angel of the Waters is not in the render.** The height probe casts **43 rays, all 43 landing on built fabric**, and measures the subject at **8.64 m** above a ground of 17.16 m. The object it measured is `lm_b_bethesda_terrace.2`, a thin upright slab **1.4 m** across this camera's bearing. The sightline then casts 13 rays, **13 clear**, **8 of them on the subject**, for a visible fraction of **0.615**, and reports `subject_visible: true`. Every one of those numbers is correct, and together they certify that a stepped base carrying a slab is visible where the photograph shows a tiered bronze fountain with a winged figure on top. **The verification passed on a stand-in.** The record is not lying; it is measuring the thing that is there.

**This sheet is also where J113's proposed repair was measured and refused.** The fan's across-extent is the probed object's, and at 1.4 m it is under the 12 m floor, so the fan is the floor — which is the fault J113 records. The repair was to take the whole model's extent instead, and the record carries what that would have been: **104.4 m across this bearing**, over **all 27 parts** of `lm_b_bethesda_terrace`, out of an axis-aligned 131.4 by 102.5 m. At 51 m that subtends more than the picture, so the fan would have been clipped to the frame — this lens spans **65.5°** horizontally — and the published fraction would have measured how much of the *frame* the terrace fills. Neither number is the subject's: the fountain is a 1.4 m object inside a landmark model that carries its own site — the terrace, the stairs, the plaza, the arcade. What the fault needs is the fountain basin's own footprint, which this build does not hold; until then the record prints both boxes side by side so the gap is visible here rather than argued elsewhere.
...
* **The Lake is drawn**, a flattened water surface of **3,737 quads**, in the middle distance where the photograph's water sits, with the far shore in the same place.
...
* **The fountain is massing, not sculpture.** No winged figure, no tiered bowls, no bronze: a stepped cylinder with a short upright block on it, in flat pink-grey, and the block the probe measured is **1.4 m** across. The photograph's subject is the most detailed object in its frame and the render's is among its least.
...
* **The Lake reads as a pale sheet rather than green reflective water, and it is far smaller in the frame than the photograph's.** The surface is there and flattened correctly; what is missing is its material, and the near ground and the canopy between them leave it a strip where the photograph gives it the middle third.
* **The canopy is a row of separate crowns, not a wall.** The photograph's far bank is one continuous mass of foliage with no sky between the trees; the render's shows sky through the gaps and each crown reads as its own object, which is what six-triangle impostor cards at a regular spacing look like at 100 m.
* **The near field is a wide expanse of dark asphalt.** The photograph stands at the plaza's own edge with patterned brick under the lens; the render puts a grey roadway surface across the bottom third before the plaza disc begins.
