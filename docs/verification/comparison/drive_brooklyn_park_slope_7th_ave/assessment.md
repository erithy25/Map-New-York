...
* **The frame is less than half as bright as the photograph and carries about a third of its colour**: mean **0.2244** against **0.4799** (**0.468×**), standard deviation **0.1588** against **0.2928** (**0.542×**), 95th percentile **0.5221** against **0.9644**, 5th percentile **0.0140** against **0.0484**. Part of that is a difference of weather and exposure — the photograph is a bright overcast with a blown white sky, the render a clear sky at an assumed instant, and a camera stops down where this renderer never does. **The chroma gap is not**: **0.0534** against **0.1478**, a ratio of **0.361**. `concrete` was moved to a scan that does not bind the albedo cap and **this frame's statistics did not change by a digit**; two surfaces still bind it (`roof_membrane`, `wood_clapboard`) and `brownstone` is drawn from a travertine scan. What the gap is made of is that `shellmat` varies a building's tone and never its hue, so a street of individually faced buildings is drawn in one palette (J66).
...
| chroma 0.361× | not the capped textures — replacing one moved this frame by nothing. `shellmat` varies tone and never hue, so every building of a class is one colour at different brightnesses (J66, second half) | **data — no source exists** |
