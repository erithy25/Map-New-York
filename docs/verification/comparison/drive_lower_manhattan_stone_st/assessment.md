...
* Both of today's changes hold up under inspection. The saloon standing up the block has nothing beneath its rear axle: the imported bone display sphere of J44 is gone. The 90 surveyed curb ramps here are in the pavement mesh rather than the prop table (J21) - though none of them is visible in this view, so what this sheet shows is the counter and not the geometry.
...
* The lit shopfront fascia is one unbroken white band on grey pilasters, running the full length of both walls at a single height, every panel blank white with no lettering on it, and one identical gooseneck downlight standing in front of each. The reference's shopfronts are individually dark, with a red awning, a projecting bracket lamp, "The DUBLINER" in gold on black and a red neon telephone number.
...
* The tall right-hand facade carries its windows as thin dark dashes at frame scale. Close in, each one does have a pale frame, a single glazing bar and a recess whose head casts a shadow onto the brick - frame, reveal and depth are all modelled - but there is no sill and no projecting trim, and at this oblique angle the wall reads as rows of dashes. The reference's blocks carry framed sash windows with maroon joinery, stone sills, iron balconies, a full-height fire escape and a slated mansard with dormers under a copper cornice. Six fire-escape kit pieces are counted in range and neither wall in view has one (A2).
* Irregular green blocks are stuck flat to the left wall at ground-floor level, below the fascia band and between the pilasters, spanning knee to head height of the pedestrian beside them - five vegetation kit pieces, reading at this range as pixelated moss, and answering to nothing in the photograph.
...
| the fascia band carries no content at all | an instanced fascia piece carries one texture, so no business name is baked onto it and every panel is lit and blank (B15a) | material |
...
| no sills, balconies or fire escape on the right-hand wall | facade pattern, material and trim are inferred from a rule table rather than observed (A2) | geometry |
