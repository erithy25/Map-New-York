...
**The place is the right place.** The camera stands on the photograph's own EXIF GPS, **20.2 m** from the item's recorded viewpoint, then walked **7.2 m** onto the nearest sidewalk polygon because that point was closed off 11 m ahead; from there the azimuth is clear for **60 m** and the nearest built thing is **`kit_1604_563` at 10.5 m**, left at **-18.1°**.

**That name is the first of its kind in this project's comparison records, and it is why this sheet is worth reading twice.** Across all 171 records of the previous pass `clearance.nearest_obstruction` named a prop 68 times, a tile shell 43 times and a landmark 32 times, and **a kit piece not once** — in scenes that place thousands of kit objects each. The clearance test predated the kit stage and kit is placed from the facade tables rather than from the props catalogue, so it passed through none of the collections the probe sampled (J117a). Kit is now counted, and the thing it names here is a **storefront**: `kit_1604` resolves to `storefront_bay_48` in `data/processed/facade/kit_catalog_map.json`, the 563rd placement of that asset, standing on the left wall of the lane where the photograph has a shopfront. The previous record named the tile shell behind it at **11.2 m**; the shopfront in front of it is the more specific and the nearer answer, and the frame shows a shopfront there.

**The light is the photograph's instant and not its weather.** The Sun is at the photograph's EXIF second, elevation **58.6°** from **204.4°** — high, behind the camera's right shoulder — and the render shows it in one place only: the upper storeys of the right wall at the top right of the frame, hard shadows falling from every sill and reveal and a diagonal shadow line from something behind the camera across them; the near-left brick is a flat sky-lit face with no lit/shade split, and the tan building at the centre stands in shade, darker than the frame's median. The photograph, under overcast, has no shadow.

**The lighting is metered, not dim.** The frame was developed at **+3.61** stops where the physical rule alone would have granted **+0.00**: the linear median **0.014769** was placed at **0.18** (J83). That is under the +4 line at which the record calls a frame under-lit. The photographer's exposure sits **-1.342** stops from the same middle-grey convention, the render's at **0.245**; the median ratio of **1.689** is that exposure difference first, the pitch second, and only then the city.
...
* **The street type is the street type.** A narrow party-wall brick lane, shopfronts under awnings, an unmarked pedestrian surface, no traffic but one vehicle where the lane meets the cross street, a taller building closing the far end. The kit carries **238 storefronts**, **44 storefront interiors**, **56 entry doors**, **4,787 windows** and **93 window accessories** — the last visible as air-conditioner boxes in the upper openings, as in the photograph.
...
* **The shadow floor is not the J83 deficit.** p05 **0.1353** against **0.0832**: the render's darkest twentieth is brighter than the photograph's, the reverse of the floor shortfall J83 recorded across the pass; the record offers no cause beyond the two exposure choices.
...
* **The photograph is aimed up and the render is level.** Reference p95 **1.0**, blown sky, against render **0.6196**, a sunlit wall; standard deviation **0.3019** against **0.1641**, ratio **0.544**. The mean ratio **1.087** is pitch and exposure cancelling.
...
`scene.agents.seconds` 543.68 -> 545.23,
`scene.seconds` 680.61 -> 680.64,
`seconds.render` 222.4 -> 217.0,
`seconds.scene` 706.2 -> 705.5,
`seconds.total` 928.6 -> 922.5.
...
One figure above is not in this render record, and it cannot be: it is what the **previous** pass
measured at the same viewpoint, quoted so the change J117a made is a before and after rather than an
assertion. It was read from this sheet's own `render.json` as it stood at commit `28b6839`, the last
state of the repository before any v17 sheet landed.

| figure | where it comes from |
|---|---|
| 11.2 | `clearance.nearest_obstruction_m` in this sheet's v16 record, beside `nearest_obstruction` `t_-6_0_red_brick`; the v17 record names `kit_1604_563` at 10.5 m instead |

The wall-clock figures in the table below are the **previous** render's, quoted so that "every measured value is identical" is a comparison a reader can check rather than a claim. They were read from this sheet's own `render.json` at commit `0f9bae0`, its last state before the pass restarted.

| figure | where it comes from |
|---|---|
| 543.68 | `scene.agents.seconds` in the previous record; the re-render took 545.23 s |
| 680.61 | `scene.seconds` in the previous record; the re-render took 680.64 s |
| 222.4 | `seconds.render` in the previous record; the re-render took 217.0 s |
| 706.2 | `seconds.scene` in the previous record; the re-render took 705.5 s |
| 928.6 | `seconds.total` in the previous record; the re-render took 922.5 s |
