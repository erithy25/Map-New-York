...
**The height probe worked perfectly and answered the wrong question.** All **43 of 43** rays landed on built fabric — as clean a reading as the instrument can give — and J74's rule fired correctly: the nearest catalogue origin is **138.0 m** away, past the 120 m the old rule looked in, so the height came from the geometry rather than from the catalogue's 84.3 m. The answer was **55.93 m** above a ground of 0.0 m, on `lm_b_brooklyn_bridge.35`, whose plan extent the record now gives as **951.7 m across this camera's bearing of 288.3° by 930.5 m along it** — longer than either axis span, because the cable runs diagonally to the view (the previous record published the axis-aligned pair, 761.6 by 728.2 m).

**That object is a cable.** Measured off `blender_out/landmarks/b_brooklyn_bridge.glb`, the four main cables — `cable_cable_t-12.6`, `t-3.1`, `t+3.1`, `t+12.6` — are each 3,516 triangles spanning **728.2 by 761.6 m** in plan and z 30.3 to 81.6 m. Those are axis-aligned spans and matched the previous record's extent to the decimetre; this record measures the same cable across the camera's bearing and gets **951.7 m**, which is the diagonal of the same box and is the number the fan uses. The item's coordinate falls under the catenary, so the ray up from it met a cable at 55.93 m and stopped. **The tower is in the same file and is not what was measured**: `tower_bk_pier` runs from −13.0 to 36.5 m on a 45.5 by 44.7 m footprint, `tower_bk_body` from 36.5 to **85.0 m** on 43.3 by 42.5 m in 270 triangles, with 456 triangles of string courses, a band at 75.9 m and four cable saddles topping out at 81.4 m. So the model's Brooklyn tower stands 85.0 m against the 84.3 m its own catalogue publishes, and the sheet reports two thirds of that because a rope was in the way.

**The cable's extent then sized the sightline.** The fan's horizontal half-angle is **27.22°**, taken from that 951.7 m width, and the record notes it was *"clipped to the frame, because a ray outside the picture tests something that is not in it"*. A fan built to span a 951 m cable cannot be a test of a 43 m tower. This is J94's fault — a joined mesh whose extent means nothing for a fan — appearing inside a **landmark** model rather than a tile mesh, and the record's `is_tile_mesh: false` reads as reassurance when it is not one. Measured across the corpus as this sheet was written — **16 of 118** records that carry a probed-object extent report one **300 m or wider, and 8 of those are landmark objects**, not tile meshes: the Bronx-Whitestone at **1,093.4 m**, the Brooklyn Bridge walkway at **1,052.2 m**, the Queensboro at **1,002.4 m**, this sheet at **951.7 m**, the Throgs Neck at 887.1 m across by **1.5 m** along, the Pulaski at 634.5 m, the Metropolitan Museum at 322.8 m and the American Museum of Natural History at 313.2 m. (The corpus is mid-pass, so these are the records that exist now and the count will move; four of the eight are already of the current generation.)
...
* **The sightline fan was sized from a 951.7 m cable** and had to be clipped to the frame, so the 0.231 visible fraction is not a test of the tower.
...
| 16 of 118 records measure a probed-object plan extent 300 m or wider, and 8 of those are landmark objects rather than tile meshes | counted over every record carrying `subject.plan_extent` at the time of writing, a corpus still mid-pass; the eight are the Bronx-Whitestone 1,093.4 m, the Brooklyn Bridge walkway 1,052.2 m, the Queensboro 1,002.4 m, this sheet 951.7 m, the Throgs Neck 887.1 m across by 1.5 m along, the Pulaski 634.5 m, the Metropolitan Museum 322.8 m and the AMNH 313.2 m |
| 84 per cent of the prop rows in range were not placed | the record's own `props.placed` of 684 against `props.rows_in_range` of 4,373, which is 84.4 per cent; the previous record's counts gave 82 |
...
| the worst park-ground clearance in the pass so far | this sheet's `under_frac` of 0.387 over 1,765 samples is the highest of the **40** records of the current generation that carry the band, ahead of the Brooklyn Bridge walkway at 0.3453 and the DUMBO Manhattan Bridge view at 0.3210; the claim is re-taken against this pass rather than inherited from the last |

## Cause of each gap

| gap | cause | class |
|---|---|---|
| 55.93 m published for an 85.0 m tower | the item's coordinate falls under the main cables' catenary, so the probe's ray met a cable first; the probe has no way to prefer masonry over a rope, and the object it hit spans 951.7 m so nothing about its extent flags the mistake (J74, J94) | **verification — open; the repair is the same as J94's, that an object this large cannot stand for the subject** |
| the fan was sized from a cable and clipped to the frame | the horizontal half-angle is taken from the hit object's plan extent, which is a 951.7 m cable (J94, extended to landmark models) | **verification — open** |
