...
* **One city surface is still drawn at the wrong level**: `roof_membrane` binds the albedo cap, and its residual is published in the record. It is the only one left — `concrete` was moved to a scan that does not bind the cap, and the catalogue records that every scan meeting a roof membrane's level is a concrete or a plaster (J66). **The frame's statistics did not move when those materials changed**, which says the colour gap below is not theirs.
...
* **The facades are more uniform than the street is, and this is what the colour gap is made of.** The classifier assigns one material family per block face, and per-building paint, siding and shopfront colour are in no source it reads. The photograph's ten metres carry painted brick, clapboard, a red timber shopfront, a striped fabric awning and a hand-lettered sign. 14 city surfaces are dressed in this frame and every one of them is a *class* of surface, not a building.
...
| chroma 0.357× | not the capped materials: replacing two of the three moved this frame's statistics by nothing at all. It is that one material family is assigned per block face, so a street of individually painted shopfronts is drawn in one palette (J66, second half) | material |
