...
**In the scene**, within 900.0 m of the camera and not all of it in frame — 6 building tiles (321,776 tris), 0 landmark models, 27,707 pavement polygons (12,707 white, 4,666 roadbed, 4,562 sidewalk, 3,619 curb, 801 crosswalk, 784 yellow, 459 median, 106 parking lot, 3 plaza), 1259 props of the 5,844 in range, 4,917 kit pieces, 65 vehicles and 407 people; 4,500,039 triangles. Ground mesh 96,800 triangles, 0 holes. 14 city surfaces are dressed from the shared photographic catalogue.
...
**Re-rendered under the eight repairs of the v17 pass, and nothing in its record moved.** Its reference photograph carries its own EXIF date, so J112 and J114 had nothing to change; its camera was already where the walk leaves it, so J115 and J116 had nothing to trigger; and nothing off a facade stands in its frame, so J117a names the same thing it named before. The figures below differ from the previous pass only in the last digit or two, which is the renderer's own sampling, and the two halves are the same two pictures. A repair that leaves a sheet alone is the repair being targeted rather than global.
...
* **Luminance, stated as stops**: development **+1.45**; exposure offsets **0.255** (render) against **-1.101** (photograph), **1.356** stops apart. Mean ratio **1.187**, median **1.561**, tonal spread **0.847**. The p05 floor, **0.0583** against **0.0426**, is inside the photographs' JPEG floor and not counted.
* **About a fifth of the props in range were placed**: **1259** of **5,844**, capped at a triangle budget of **1,313,553**; the kit was capped at **1,357,846**.
...
`scene.agents.seconds` 375.27 -> 348.72,
`scene.seconds` 501.35 -> 480.41,
`seconds.render` 266.9 -> 208.1,
`seconds.scene` 510.8 -> 485.0,
`seconds.total` 777.8 -> 693.1.



## Measured for this assessment

The wall-clock figures in the table below are the **previous** render's, quoted so that "every measured value is identical" is a comparison a reader can check rather than a claim. They were read from this sheet's own `render.json` at commit `4a55887`, its last state before the pass restarted.

| figure | where it comes from |
|---|---|
| 375.27 | `scene.agents.seconds` in the previous record; the re-render took 348.72 s |
| 501.35 | `scene.seconds` in the previous record; the re-render took 480.41 s |
| 266.9 | `seconds.render` in the previous record; the re-render took 208.1 s |
| 510.8 | `seconds.scene` in the previous record; the re-render took 485.0 s |
| 777.8 | `seconds.total` in the previous record; the re-render took 693.1 s |
