...
* **The frame is still much darker than the photograph and carries under two fifths of its colour**: mean **0.1727** against **0.6174** (**0.280×**), standard deviation **0.1336** against **0.2657** (**0.503×**), 95th percentile **0.4706** against **0.9184**, 5th percentile **0.0227** against **0.1440**. Most of the luminance gap is the pairing: the photograph is a sunlit elevation with a bright sky filling its upper third, the render looks along a street whose near half is in its own shade. **The chroma gap is not** — **0.0509** against **0.1352**, a ratio of **0.376**. 17 city surfaces are dressed here and two of them still bind the albedo cap (`roof_membrane`, `wood_clapboard`); `concrete` was moved to a scan that does not, and **the frame's mean and chroma did not move at all** when it was. What is left is that one material family is assigned per block face, so a street of individually faced buildings is drawn in one palette (J66, second half).
...
| chroma 0.376× | not the capped textures — replacing one of the three moved this frame by nothing. One material family is assigned per block face (J66, second half) | material |
