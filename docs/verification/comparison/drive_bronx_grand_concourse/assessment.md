...
* The far corner carries a second green sidewalk shed in the same vocabulary as the near one: a netted green parapet over a timber deck fascia, green posts, a white lit strip under the deck, six or seven people standing beneath it. Behind the posts is plain beige wall - no glazing, no display bays, no entrances - so nothing in this frame is a shopfront, and the 329 storefronts and 56 storefront interiors in the record are placed where the picture cannot show them. The photograph carries the same green band along the base of its block.
...
* **J44 holds on the three vehicles this frame contains**: a yellow taxi, a white box truck and a dark car mostly hidden behind the near column, all at the far junction and each a few dozen pixels tall. The taxi's rear wheel and the truck's rear axle block are legible enough to say they are clean, with no untextured grey sphere under either. The other 86 vehicles are out of frame and this sheet says nothing about them.
* One other repair shows: 6 pedestrians were culled over the observer after the camera moved (I15), leaving one figure near the lens where the previous render had four backs. The J21 curb-ramp repair is in the record only - `curb_ramp: 81` now sits under `built_elsewhere` with the unmapped list empty - and this frame cannot confirm it, for the reason given below.
* The pavement stack reads correctly where visible - concrete footway, a raised kerb catching the sun, a dark gutter line, asphalt - with a red hydrant at a tree pit and base plates under the shed posts. Both halves share their weather and light: clear sky, high sun behind the lens, hard-edged shadows thrown away from the camera and into the picture - in the render the near figure's legs and briefcase drop theirs past his feet, deeper into the frame, and fabric behind the lens shades the foreground roadway.
