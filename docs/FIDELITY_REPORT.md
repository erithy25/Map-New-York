...
Generated 2026-09-07 10:23 UTC from commit `e2a237fc4206` by `pipeline/nycsim_pipeline/report/fidelity.py`.
...
| B3 | **Stepped massing was written and never applied.** 28 % of the city (301,311–307,735 buildings) has two or more roof levels with a median 4.44 m step, and every setback tower shipped as a single slab. The code to build them — `roofsteps.py`, `shellgeom._build_stepped`, `tiledata._steps_for` — was already on the branch, and **only 8 of the 920 tiles had ever been rebuilt with it**, covering 2,105 buildings. So this was not an unbuilt feature; it was a built feature the city did not carry, which is worse, because nothing in the repository said so — `docs/verification/buildings_mesh/REPORT.md` §7 presented it as unbuilt and explained why. | The level *outlines* were the missing input: the tables publish level height and level area but not the level outline, and splitting a footprint by area alone means guessing which limb is the low one. The outlines are recoverable from the CityGML `RoofSurface` triangles already on disk, grouped by z and unioned — verified independently in this pass: the triangle blobs are in NYC_TM and reproduce each row's own bounding box to 0.0 m, roof triangles are exactly horizontal and walls exactly vertical, and 99.1 % of recovered level heights land within 0.5 m of the published `roof_level_z`. | **In progress at the time of writing**, together with the material work (B12) so the shells are re-exported once rather than twice. The pass also caught a case where the feature would have made things *worse*: on the Bayside tile 275 of 1,474 pitched buildings were having their gable replaced by flat stepped levels for the sake of a porch-sized step, because `_build_stepped` caps every level flat. A pitched building now keeps its roof unless the lowered part is at least a quarter of its plan, so 177 of those 275 keep their gables. |
