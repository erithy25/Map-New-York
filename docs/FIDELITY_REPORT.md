...
Generated 2026-09-07 08:35 UTC from commit `2763316cb1c7` by `pipeline/nycsim_pipeline/report/fidelity.py`.
...
| I12 | **Seven of the 57 sheets compare two halves that do not show the same subject** — measured across the written verdicts, not estimated. Two of the seven are *mandated* viewpoints. The list: `fifth_ave_42nd_north` ("the two halves face different ways and cannot be compared on composition — the item says 'looking north up Fifth Avenue' and all three photographs collected for it are pictures of the library facade looking west"), `fifth_ave_42nd_south` ("the two halves face different streets"), `landmark_washington_square_arch` ("the photograph does not contain the Washington Square Arch: it looks the other way"), `drive_bronx_arthur_ave`, `drive_bronx_grand_concourse`, `drive_brooklyn_bed_stuy_stuyvesant_ave` and `drive_lower_manhattan_stone_st`. Separately, the Williamsburg Bridge camera sits ~500 m too far back. | Two faults in the reference chooser, both upstream of the comparison lane. The heading comes from a camera-GPS-to-subject bearing that assumes a photograph taken *at* a viewpoint is a photograph *of* it — which is false for a street-axis subject, where the photographer is standing on the axis and shooting across it. And a 250 m rule rejected the Williamsburg photograph's own GPS, correct at 117 m from the subject, in favour of a nominal viewpoint that was not, at 506 m. The chooser also scored one photograph 14.2 on a subject it does not show. | A subject test that checks the photograph *contains* the subject rather than that it was taken near it, and a distance rule that prefers a measured GPS over a nominal viewpoint. Both would re-pick photographs across 172 subjects, so any change needs the affected scenes re-rendered and re-assessed rather than silently re-picked. **In progress at the time of writing.** |
