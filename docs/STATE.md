...
| `data/processed/tiles/{tile}/props.parquet` (`rule:woodland_canopy`) | 1,752,719 declared-procedural canopy stems inside the OSM wood/forest/scrub polygons (Stage 55, J86): 1,680 tiles, 164.8 MB; the count is a consequence of crown closure (wood, forest) and of the 3 m spacing constant (scrub), never a measurement; a rebuild is byte-identical |
| `blender_out/tiles/{tile}/tile_parkground.glb` | 133 tiles (the 43 first-drive tiles + the 90 the 23 park sheets reach), 205 MB, pavement and open water subtracted; the verification scene loads them (`scene.add_parkground`); city-wide park ground for the remaining ~800 park tiles is a named remainder (disk) |
