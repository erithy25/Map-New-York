...
def write_directory_license_records() -> list[Path]:
    """Write one aggregate ``LICENSE.json`` per audio directory from ``assets/audio/CATALOG.json``.

    ``pipeline/nycsim_pipeline/report/asset_licenses.py`` scans ``assets/**`` and expects a ``LICENSE.json`` in
    every directory that holds payload files; the per-file ``*.license.json`` sidecars this script writes are
    richer but are not what that scanner looks for. This function bridges the two: it groups the catalogue by
    directory and writes a record carrying the fields the scanner reads (``name``, ``license``, ``url``,
    ``author``, ``fetched_at``, ``sha256`` as a file -> hash mapping) plus the full per-file detail, so nothing is
    lost and nothing has to be stated twice.

    Returns the list of files written.
    """
    if not CATALOG_PATH.exists():
        raise FetchError(f"{CATALOG_PATH} does not exist; run the fetcher first")
    ...
    for entry in catalog.get("files", []):
        ...
        if not rel:
            continue
        # Radio tracks are recorded relative to assets/audio/radio (stations.json's ``audio_root``); SFX are
        # recorded relative to assets/audio. Resolve against both and keep whichever exists.
        candidates = [RADIO_ROOT / rel, AUDIO_ROOT / rel]
        path = next((c for c in candidates if c.exists()), None)
        if path is None:
            log.warning("catalogue entry %s has no file on disk; skipped in the licence record", rel)
            continue
        by_dir.setdefault(path.parent, []).append(entry)

    written: list[Path] = []
    for directory, entries in sorted(by_dir.items()):
        entries = sorted(entries, key=lambda e: e["file"])
        licences = sorted({e.get("licence", "unstated") for e in entries})
        authors = sorted({(e.get("artist") or "").strip() for e in entries if (e.get("artist") or "").strip()})
        sites = sorted({e.get("source_site", "") for e in entries if e.get("source_site")})
        record = {
            "schema_version": SCHEMA_VERSION,
            "name": f"NYCSim audio: {directory.relative_to(REPO_ROOT / 'assets').as_posix()}",
            "license": licences[0] if len(licences) == 1 else "; ".join(licences) + " (per file)",
            "licenses": licences,
            "author": "; ".join(authors[:8]) + (" and others" if len(authors) > 8 else ""),
            "url": entries[0].get("source_page") or entries[0].get("source_url", ""),
            "sources": sites,
            "fetched_at": catalog.get("generated_at", ""),
            "sha256": {Path(e["file"]).name: e.get("sha256", "") for e in entries},
            "note": ("Only CC0 / Public Domain / CC BY files are accepted; share-alike, non-commercial and "
                     "no-derivatives are rejected by unreal/tools/fetch_radio.py. Per-file provenance is in the "
                     "matching <file>.license.json."),
            "files": [
                {"file": Path(e["file"]).name, "title": e.get("title", ""), "artist": e.get("artist", ""),
                 "licence": e.get("licence", ""), "licence_url": e.get("licence_url", ""),
                 "source_url": e.get("source_url", ""), "source_page": e.get("source_page", ""),
                 "source_site": e.get("source_site", ""), "duration_s": e.get("duration_s", 0.0),
                 "bytes": e.get("bytes", 0), "sha256": e.get("sha256", "")}
                for e in entries
            ],
        }
        directory.mkdir(parents=True, exist_ok=True)
        out = directory / "LICENSE.json"
        with open(out, "w") as f:
            json.dump(record, f, indent=1, sort_keys=True)
        written.append(out)

    # The two index files this script generates (stations.json, CATALOG.json) are project output, not third-party
    # content, but they still sit in directories the licence scanner walks, so they get their own record.
    for directory, name, contents in ((RADIO_ROOT, "NYCSim radio station index", "stations.json"),
                                      (AUDIO_ROOT, "NYCSim audio catalogue", "CATALOG.json")):
        if not (directory / contents).exists():
            continue
        record = {
            "schema_version": SCHEMA_VERSION,
            "name": name,
            "license": "Generated by this repository (see the project licence); indexes third-party audio whose "
                       "own licences are recorded per directory and per file",
            "author": "NYCSim build",
            "url": "",
            "fetched_at": catalog.get("generated_at", ""),
            "sha256": {contents: hashlib.sha256((directory / contents).read_bytes()).hexdigest()},
            "note": "Not third-party content: a manifest of the files whose licences are recorded alongside them.",
        }
        out = directory / "LICENSE.json"
        with open(out, "w") as f:
            json.dump(record, f, indent=1, sort_keys=True)
    ...
    def write_outputs(self, stations: list[tuple[StationSpec, list[Track]]], sfx: list[Track]) -> None:
        ...
        for written in write_directory_license_records():
            log.info("licence record %s", written.relative_to(REPO_ROOT))
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--relicense", action="store_true",
                    help="only rewrite the per-directory LICENSE.json files from the existing CATALOG.json")
